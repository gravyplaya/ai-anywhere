...
export function initChatTab(el: HTMLElement): TabLifecycle {
  ...
  container
    .querySelector("#chat-get-started-btn")!
...
async function toggleTools(): Promise<void> {
  ...
  if (badgeEl) {
    ...
    badgeEl.textContent = toolsEnabled
...
async function registerDemoTools(): Promise<void> {
  const { ToolCalling, toToolValue } =
    await import("../../../../../sdk/runanywhere-web/packages/llamacpp/src/index");
  ...
  ToolCalling.registerTool(
    {
      name: "get_weather",
      description:
        "Gets the current weather for a given location. Returns temperature, condition, humidity, and wind speed.",
      parameters: [
        {
          name: "location",
          type: "string",
          description:
            'The city name to get weather for (e.g., "San Francisco")',
          required: true,
        },
      ],
      category: "Utility",
    ...
    async (args): Promise<Record<string, ToolValue>> => {
      ...
      try {
        ...
        return {
          ...
          temperature_fahrenheit: toToolValue(
            (current.temperature_2m * 9) / 5 + 32,
          ),
          condition: toToolValue(
            conditionMap[current.weather_code] ?? "Unknown",
          ),
  ...
  ToolCalling.registerTool(
      ...
      parameters: [
        {
          ...
          type: "string",
          description:
            'IANA timezone (e.g., "America/New_York"). Defaults to local timezone.',
          required: false,
        },
      ],
      category: "Utility",
    ...
    async (args) => {
      const tz =
        args.timezone?.type === "string" ? args.timezone.value : undefined;
      const now = new Date();
  ...
  ToolCalling.registerTool(
    {
      name: "calculate",
      description:
        "Evaluates a mathematical expression and returns the result. Supports basic arithmetic (+, -, *, /, ^), parentheses, and common functions.",
      parameters: [
        {
          name: "expression",
          type: "string",
          description:
            'The math expression to evaluate (e.g., "2 + 3 * 4", "sqrt(16)", "(10 + 5) / 3")',
          required: true,
        },
      ],
      category: "Utility",
    ...
    async (args): Promise<Record<string, ToolValue>> => {
      const expr =
        args.expression?.type === "string" ? args.expression.value : "";
      try {
  ...
  console.log(
    "[Chat] Demo tools registered: get_weather, get_current_time, calculate",
  );
}
...
async function sendStreaming(text: string, loaded: ModelInfo): Promise<void> {
  ...
  const {
    ...
    cancel,
  } = await TextGeneration.generateStream(text, {
  ...
  const { bubbleEl, thinkingWrapper, rowEl } =
 ...
 */
async function sendWithToolCalling(
  text: string,
  loaded: ModelInfo,
): Promise<void> {
  const { ToolCalling } =
  ...
  const toolCallInfos: ToolCallInfo[] = result.toolCalls.map((tc, i) => {
    ...
    return {
      ...
      result: tr
        ? JSON.stringify(
            tr.success ? resultPlain : { error: tr.error },
            null,
            2,
          )
        : undefined,
...
function toolValueToPlain(tv: { type: string; value?: unknown }): unknown {
  switch (tv.type) {
    ...
    case "null":
      return null;
    case "array":
      return (tv.value as unknown[]).map((v) =>
        toolValueToPlain(v as { type: string; value?: unknown }),
      );
    case "object": {
      const obj: Record<string, unknown> = {};
      for (const [k, v] of Object.entries(
        tv.value as Record<string, unknown>,
      )) {
...
function renderStreamingBubble(msg: ChatMessage): {
  ...
  const thinkingWrapper = row.querySelector<HTMLElement>(
    `#streaming-thinking-${msg.id}`,
  )!;
  const bubbleEl = row.querySelector<HTMLElement>(
    `#streaming-bubble-${msg.id}`,
  )!;
